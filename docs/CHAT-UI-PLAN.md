...
| C27 | Error handling & empty states | P1 | ✅ DONE | Global error boundary with recovery. Toast notifications for API errors. Offline detection banner. Empty states for all list views (no agents, no sessions, no integrations). Loading skeletons. **Done 2026-03-08:** Zustand `toastStore` with 4 variants (success/error/warning/info), auto-dismiss timers, max 5 visible, convenience `toast.*` helpers. `ToastContainer` component (fixed bottom-right desktop / bottom-center mobile, animated entry/exit, OKLCH-themed variant colors for dark+light, dismiss button, `role="alert"` + `aria-live`). `OfflineBanner` component with `useOnline` hook (`useSyncExternalStore` on `online`/`offline` events), persistent warning bar above TopBar. Shared `EmptyState` component (icon + title + description + optional CTA, compact variant for panels). API client (`api.ts`) wired with automatic toast dispatch: network errors (TypeError → "Network error"), 5xx → "Server error", 429 → "Rate limited". `AgentList` and `IntegrationGrid` refactored to use shared `EmptyState`. `AppShell` updated with `OfflineBanner` + `ToastContainer`. ErrorBoundary already existed from C26. Loading skeletons already existed across all list views from earlier phases. Build clean (1130 KB JS across 19 chunks). |
| C28 | Production deployment | P0 | ✅ DONE | Vite build → `web/dist/`. Caddy serves `web/dist/` (update root path). Gzip/Brotli compression. Hashed asset filenames with cache-forever headers. CSP headers. GitHub Actions CI: lint + type-check + build on push to `feat/chat-ui`. **Done 2026-03-08:** Caddy config updated: `root * web/dist`, `try_files {path} /index.html` SPA fallback, `encode zstd gzip` compression, `@hashedAssets path /assets/*` with `max-age=31536000, immutable`, no-cache on `index.html` and `sw.js`. Added `/api/v1/*` reverse proxy to Go gateway (port 18796). Full security headers: HSTS (preload), X-Content-Type-Options, X-Frame-Options SAMEORIGIN (for browser iframe), Referrer-Policy, Permissions-Policy, CSP (self + wss + Stripe checkout/billing + canary iframe). GitHub Actions CI (`.github/workflows/chat-ui-ci.yml`): Node 22, npm ci, lint → typecheck → build pipeline, bundle size summary, artifact upload on main. All 28 tasks complete — Phase 6 done. |
...
| 2026-03-08 | C27 complete: Error handling & empty states. toastStore (Zustand, 4 variants, auto-dismiss, `toast.*` helpers). ToastContainer (animated, OKLCH-themed, aria-live alerts, max 5 visible). OfflineBanner + useOnline hook (useSyncExternalStore). Shared EmptyState component (icon/title/desc/CTA, compact variant). API client auto-toasts on 5xx, 429, and network errors. AgentList + IntegrationGrid refactored to EmptyState. AppShell wired with OfflineBanner + ToastContainer. |
| 2026-03-08 | C28 complete: Production deployment. Caddy config updated: root → `web/dist`, SPA fallback (`try_files`), zstd+gzip compression, immutable caching on `/assets/*`, no-cache on index.html + sw.js, `/api/v1/*` proxy to Go gateway. Full security headers (HSTS preload, CSP with self+wss+Stripe+canary iframe, X-Content-Type-Options, X-Frame-Options SAMEORIGIN, Referrer-Policy, Permissions-Policy). GitHub Actions CI workflow (lint → typecheck → build, bundle size report, artifact upload). All 28 tasks across 6 phases complete. 🎉 |
